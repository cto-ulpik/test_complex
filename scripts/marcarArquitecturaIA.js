#!/usr/bin/env node
/**
 * Script para usar IA y marcar respuestas correctas SOLO para Arquitectura de Computadoras
...
const MATERIA_NOMBRE = 'Arquitectura de Computadoras';
...
async function analizarPreguntaConIA(pregunta, respuestas) {
    ...
    const prompt = `Eres un experto en Arquitectura de Computadoras. Analiza la siguiente pregunta de examen y determina cuál es la respuesta correcta.
...
INSTRUCCIONES:
1. Analiza la pregunta cuidadosamente desde el punto de vista de Arquitectura de Computadoras
    ...
    try {
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Eres un experto en Arquitectura de Computadoras. Solo respondes con la letra de la respuesta correcta si estás 100% seguro, o 'INCIERTO' si no puedes determinarlo con certeza."
                },
                {
                    role: "user",
                    content: prompt
